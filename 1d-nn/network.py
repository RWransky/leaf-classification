...
class Network():
    def __init__(self, is_training):
        # Input data.
        self.data = tf.placeholder(shape=[None, 1, num_features, 1], dtype=tf.float32, name='input')
        ...
        self.layer1_weights = tf.Variable(tf.truncated_normal(
            [1, filter_size, 1, num_hidden], stddev=0.1),
            name="layer1_weights")
        self.layer2_weights = tf.Variable(tf.truncated_normal(
            [filter_size, filter_size, num_hidden, num_hidden], stddev=0.1),
            name="layer2_weights")
        self.layer3_weights = tf.Variable(tf.truncated_normal(
            [16 // 4 * num_features // 4 * num_hidden, num_hidden], stddev=0.1),
            name="layer3_weights")
